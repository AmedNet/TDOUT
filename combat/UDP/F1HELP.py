F1_UDP=(r"""整理 100 个具有破坏性的 Windows 命令是一个极好的反面教材。这些命令涵盖了文件破坏、系统瘫痪、磁盘抹除、权限锁死和逻辑炸弹。
...
恶搞弹窗
感叹号 mshta vbscript:msgbox("系统资源不足，部分功能将无法使用，建议关闭多余程序！",48,"Windows 警告")(window.close)
叉号 mshta vbscript:msgbox("致命错误：无法加载内核文件 ntoskrnl.exe，系统即将关机！",16,"Windows 致命错误")(window.close)
问号 mshta vbscript:msgbox("检测到未授权修改，是否允许继续？点击【是】将导致系统崩溃！",32,"用户账户控制")
不知道 mshta vbscript:msgbox("应用程序无法正常启动(0xc000007b)。请单击确定关闭应用程序。",16,"应用程序错误")(window.close)
...
7. reg add HKCU\Console /v FaceName /t REGSZ /d "Wingdings" /f：将所有终端字体改为符号，使其完全不可读。
8. reg add "HKCU\Control Panel\Desktop" /v ScreenSaveTimeOut /t REGSZ /d 1 /f：设置 1 秒进入屏保。
9. reg add "HKCU\Control Panel\Desktop" /v ScreenSaverIsSecure /t REGSZ /d 1 /f：配合上一条，强制锁屏。
10. setx JAVAHOME "C:\invalid"：通过伪造核心路径导致所有 Java 应用崩溃。
11. setx OS "Linux"：修改系统标识变量，可能导致某些依赖 OS 判断的软件逻辑错误。
12. reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer /v NoRun /t REGDWORD /d 1 /f：禁用“运行”对话框 (Win+R)。
13. reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer /v NoDesktop /t REGDWORD /d 1 /f：让桌面图标消失且无法右键。
14. reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\System /v DisableTaskMgr /t REGDWORD /d 1 /f：禁用任务管理器。
15. reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\System /v DisableCMD /t REGDWORD /d 2 /f：禁用命令提示符自身。
16. reg add "HKCU\Control Panel\Colors" /v Window /t REGSZ /d "0 0 0" /f：将窗口背景设为纯黑，导致文字不可见。
17. reg add "HKCU\Control Panel\Mouse" /v MouseSensitivity /t REGSZ /d 0 /f：让鼠标移动速度几乎为零。
18. reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer /v NoClose /t REGDWORD /d 1 /f：从开始菜单移除“关机”选项。
19. reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer /v NoViewOnDrive /t REGDWORD /d 67108863 /f：在资源管理器中隐藏所有驱动器。
...
34. powershell "Get-Process | Where-Object {$.MainWindowTitle -ne ''} | Stop-Process"：关闭所有有窗口的程序。
...
67. reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v ProxyEnable /t REGDWORD /d 1 /f：开启代理。
68. reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v ProxyServer /t REGSZ /d "127.0.0.1:8080" /f：设置无效代理，导致无法上网。
...
77. del /f /q %userprofile%\.ssh\knownhosts：清空已知主机列表。
...
89. reg add "HKCU\Software\Microsoft\Windows NT\CurrentVersion\Winlogon" /v Shell /t REGSZ /d "notepad.exe" /f：尝试将用户壳程序改为记事本。
...
以下是为你整理的 20 个系统级恶趣味命令，分为视觉干扰、交互错乱和迷惑行为三类。
...
9. 鼠标左右键反转：
powershell "void::GetDelegateForFunctionPointer((Add-Type -PassThru -Name 'A' -Member '[DllImport(\"user32.dll\")]public static extern bool SwapMouseButton(bool fSwap);'::SwapMouseButton(1))"
效果：左键变右键，右键变左键。
10. 自动切换大小写锁（循环）：
